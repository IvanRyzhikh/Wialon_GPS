...
namespace law::socket::net::detail {
    ResolvedIPRet IpResolver::resolve(std::string_view address) noexcept {
        ...
        if (isHostname(address)) {
            ...
            if (!utils::validateDNS(domain)) {
                return fromLawError<ResolvedIP>(RetCode::INCORRECT_HOSTNAME);
            }
    ...
    ResolvedIPRet IpResolver::hostnameToIp(std::string_view hostname) noexcept {
        if (auto he = gethostbyname(hostname.data()); he) {
            ...
            return fromLawError<ResolvedIP>(RetCode::INVALID_HOSTNAME_RESOLUTION);
        }
    }
}
