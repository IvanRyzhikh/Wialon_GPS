...
namespace law::socket::net::udp::detail {
    ...
    public:
        ...
        WriteCountRet write(std::string_view data, const SendOption &so = SendOption(0)) const noexcept;

        WriteCountRet write(const void *buf, std::size_t size, const SendOption &so = SendOption(0)) const noexcept;
